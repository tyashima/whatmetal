...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        ...
        model = metalnet_simple(input_shape)
        ...
        cp_cb = keras.callbacks.ModelCheckpoint("./training/simplest_cnn_model_simple.hdf5", monitor='loss', verbose=0, save_best_only=True, save_weights_only=True, mode='auto')
        cbks = [tb_cb, cp_cb]
        history = model.fit(train_image, train_label, shuffle=True, nb_epoch=100, batch_size=25, verbose=1, callbacks=cbks)
        ...
        with open(working_dir + '/training/simplest_cnn_model_simple.json', 'w') as f:
            f.write(json_string)
    ...
    for i in range(len(ans)):
        ...
        root_path = "/home/yashima/public_html/"

        path = "result_simple/" + str(i) + "_" + ans[i] + "_as_" + pred[i] + ".jpg"
        scipy.misc.imsave(root_path + path, np.asarray(test_show_imgs[i]))

        Images = '<img src="%s"align="left">' % (path, )
        html += Images
        html += '<font size="4">[Answer]:%s<br>[Predicted]:%s<font><br>' % (ans[i], pred[i])
        html += '<br><br clear="all"><br>'

    #html_file = result_dir + "/result_latest.html"
    html_file = "/home/yashima/public_html/result_simple.html"
